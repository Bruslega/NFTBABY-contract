...
contract NFTBABY is IBEP20, Ownable {
    ...
    mapping(address => uint256) public _tBalances;
    ...
    uint256 private _buyTaxFee = 2; // Reflections
    uint256 private _buyBurnFee = 1;
    uint256 private _buyLiquidityFee = 3;
    uint256 private _buyMarketingFee = 7;
    uint256 private _buyBuybackFee = 2;
    ...
    string private constant _symbol = "NFTY";
    uint8 private constant _decimals = 9;
    uint256 private constant _tTotalSupply = 1000000000000 * 10 ** 9;
    // uint256 private constant _tTotalSupply = 100000000 * 10 ** 9;
    ...
    uint256 public _tMarketingTotal;
    uint256 public _tBurnTotal;
    uint256 public _tBuybackTotal;
    ...
    constructor() {
        ...
        address _pancakePair = IPancakeFactory(_pancakeRouter.factory()).createPair(_pancakeRouter.WETH(), address(this));
        ...
        _isExcludedFromFee[_pancakePair] = true;
    ...
    function reflectionFromToken(uint256 tAmount, bool deductTransferFee) public view returns (uint256) {
        ...
        if (!deductTransferFee) {
            (rValues memory _rv,) = _getValues(tAmount);
            return _rv.rAmount;
        } else {
            (rValues memory _rv,) = _getValues(tAmount);
    ...
    function _getTValues(uint256 tAmount) private view returns (tValues memory) {
        ...
        _tv.tTransferAmount = tAmount
            .sub(_tv.tFee)
            .sub(_tv.tLiquidity)
            .sub(_tv.tMarketingFee)
            .sub(_tv.tBurnFee)
            .sub(_tv.tBuybackFee);
        return (_tv);
    }
    ...
    function _getRValues(uint256 tAmount, uint256 tFee, uint256 tLiquidity, uint256 tMarketingFee, uint256 tBurnFee, uint256 tBuybackFee, uint256 currentRate) private pure returns (rValues memory) {
        ...
        _rv.rTransferAmount = _rv.rAmount
            .sub(_rv.rFee)
            .sub(_rv.rLiquidity)
            .sub(_rv.rMarketingFee)
            .sub(_rv.rBurnFee)
            .sub(_rv.rBuybackFee);
        return (_rv);
    }
    ...
    function swapTokens(uint256 marketingTokensIn) private {
        _takeLiquidity(marketingTokensIn);

        (
            uint112 reserve0,
            uint112 reserve1,
        ) = IPancakeV2Pair(pancakePair).getReserves();

        if (
            (reserve0 == 0 && reserve1 == 0)
            || marketingTokensIn < 1 * 10 ** 8
        ) return;
